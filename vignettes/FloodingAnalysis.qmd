...
In conclusion, across various analytical approaches—including basic time series techniques like cross-correlation functions (CCF), generalized additive mixed models (GAMMs), vector autoregressive (VAR) modeling, and Granger causality tests—the findings consistently indicate that, in addition to the storm surge's own past values, wind speed plays a statistically significant role in explaining variations in storm surge. This alignment across models strengthens the evidence that wind is a key driver of storm surge dynamics.
