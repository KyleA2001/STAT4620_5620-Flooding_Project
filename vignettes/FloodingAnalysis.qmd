...
This study investigates the relationship between climatic variables—wind speed, temperature, and rainfall—and storm surge levels at two coastal locations in Nova Scotia: Halifax and Yarmouth. Various time series analysis techniques are employed to explore covariate interactions and their influence on surge dynamics. Results from this indicated that wind speed is the most significant covariate that has an impact on the surge dynamics. Then Extreme Value Analysis (EVA) is employed to quantify the extreme storm surge levels for different return periods, with and without covariate incorporation. Achieved results provide initial insights into surge–climate interactions and storm surge return levels while suggesting the need for further refinement of modeling approaches to inform more effective estimates for appropriate adaptation strategies for coastal risk management in Atlantic Canada.
...
Nova Scotian coastal communities are becoming increasingly vulnerable to the impacts of climate change, as rising sea levels and more frequent extreme weather events elevate the risks of flooding, infrastructure damage, and ecosystem disruption^[@R-Helderop2019]. In densely populated coastal areas, urban development has often reduced natural protective barriers, thereby heightening exposure to geophysical hazards^[@R-Vonstorch2008]. Nova Scotia, with its extensive Atlantic coastline, is no exception—having experienced a marked increase in heavy rainfall, strong winds, and severe flooding over the past decade^[@R-Childs2017]. As climate change continues to alter atmospheric and oceanic systems^[@R-Simonovic2012], providing predictions of the magnitude of coastal flooding in this region has become critically important.

One of the primary contributors to coastal flooding is high water levels, which is related to storm surge as will be explained in the Data section, which are primarily caused by high winds and low atmospheric pressure during severe weather events^[@R-mcinnes2003impact]. While wind speed is widely recognized as the dominant factor influencing water level height, other variables such as atmospheric pressure, sea surface temperature, and precipitation also play important roles^[@R-Resio2008]. For example, warming ocean temperatures have been associated with more intense storm systems^[@R-Needham2015], while rainfall—although a major factor in overall flood risk—tends to have a weaker direct relationship with the storm dynamics^[@R-Fang2021]. Understanding the combined influence of these factors is essential for improving flood prediction models and enhancing coastal resilience.

Modeling such extreme weather events is inherently challenging due to their rarity and the complex interplay of multiple climatic variables. Although these events are infrequent, their consequences are often severe, causing significant social and economic disruption. This underscores the need for more accurate analytical and forecasting tools.

The objective of this research is to examine the relationship between key climatic variables—specifically wind speed, temperature, and rainfall—and storm surge levels at two locations in Nova Scotia: Halifax and Yarmouth. The study is driven by the following research questions:

- What is the relationship between various climatic factors and storm surge levels across Nova Scotia?

- How can the most strongly correlated factors be used to improve predictions of future coastal flooding events in the region?

To provide a structured analysis, Section 2 presents the data, Section 3 outlines the statistical approaches, Section 4 demonstrates the code behind the analysis, Section 5 displays the results and their corresponding interpretations, and Section 6 concludes the research and states some points of discussion. 

# 2. Data 

## 2.1 Description

The data used in this study were compiled from multiple sources, including Environment and Climate Change Canada^[@R-weatherstats2024], and the Government of Canada^[@R-dfo_station_inventory]. Specifically, water level data were retrieved from the Government of Canada, while wind speed, temperature, and rainfall data were sourced from Environment and Climate Change Canada.
...
In this analysis, daily average values of storm surge levels are computed and used alongside the corresponding daily average values of wind speed, rain, and temperature. After preprocessing, the final dataset includes 18,569 observations for Halifax and 19,582 for Yarmouth. The daily storm surge level and its associated covariates for Halifax are presented in @fig-ts_halifax below. From the plots, it appears that storm surge level and wind speed exhibit a similar pattern, especially when compared to the other two variables. The corresponding visualization for Yarmouth is provided in @fig-ts_yarmouth in the appendix. 
...
where $f_Y$ is the distribution of the response variable, $g$ is the link function, $f_i$ is the smooth function, $K$ is the number of smooth functions, $X\beta$ is the fixed effects, and $Zw$ is the random effects.
...
A VAR model is a more generalized version of the univariate autoregressive model^[@R-hyndman2018forecasting]. It comprises one equation per variable in the system including a constant and lags of all of the variables in the system.
...
Granger Causality is a statistical technique that determines whether one time series can predict another^[@R-seth2007granger]. In a VAR($k$) model with $p$ variables, variable `x` is said to Granger-cause variable `y` if the past values of `x` provide statistically significant information about the future values of `y` that cannot be explained by the past values of `y` by itself. The test determines whether the coefficients for the lagged values of `x` are jointly significant in the equation for `y`.  If they are, we reject the null hypothesis, which holds that `x` does not cause `y`. However, it is important to note that Granger causality identifies predictive relationships rather than direct causal links.
...
The test for Granger causality in the above model involves whether a specific variable $x_j$ Granger-causes $y_t$, the null hypothesis is as follows:
...
With that, we have $(1-\alpha)\times 100\%$ confidence intervals constructed using,
...
# Storm surge
hf_acf_surge = acf(halifax$avg_water_surge, lag.max = 50, 
                   main="Halifax-ACF-Storm Surge")

# Wind speed
hf_acf_wind = acf(halifax$avg_wind_speed, lag.max = 50, 
                  main="Halifax-ACF-Wind Speed")

# Temperature
hf_acf_temp = acf(halifax$avg_temperature, lag.max = 50, 
                  main="Halifax-ACF-Temperature")

# Rain
hf_acf_rain = acf(halifax$rain, lag.max = 50, 
                  main="Halifax-ACF-Rain")
```
...
hf_ccf_surge_wind = ccf(halifax$avg_water_surge, halifax$avg_wind_speed, 
                        lag.max = 50, plot=F)

# Storm surge and Temperature
hf_ccf_surge_temp = ccf(halifax$avg_water_surge, halifax$avg_temperature, 
                        lag.max = 50, plot=F)

# Storm surge and Rain 
hf_ccf_surge_rain = ccf(halifax$avg_water_surge, halifax$rain, 
                        lag.max = 50, plot=F)
```

**GAMMs** incorporate smooth terms for temperature, wind speed, rainfall, and time, allowing for the capture of seasonal patterns and nonlinear trends in water surge dynamics. The model aims to investigate the relationship between water surge and climatic predictors, accounting for location as a random effect, which can be expressed by the following formula:
...
where $\beta_k$ is a coefficient, $f_j$ is a smooth function, and $w_{g,i}\sim N(0, \sigma^2_g)$ is a random effect covariate accounting for variations in storm surge across different locations.
...
- Secondly, a residual analysis was performed for both smooth and random effects, particularly the Residuals vs. Fitted plot and the Q-Q plot of the residuals.
...
halifax_var <- new_halifax_df_interpolation[, c("avg_water_surge", 
                                                "avg_temperature", 
                                                "avg_wind_speed", 
                                                "rain")]
...
# Residual ACF plot
acf(residuals(halifax_var_model)[,1], 
    main="VAR ACF Residuals Model 1 - Halifax")
```
...
# Residual ACF plot
acf(halifax_var_model2$varresult$avg_water_surge$residuals, 
    
    main="VAR ACF Residuals Model 2 - Halifax")
```
...
# from the GEV fit with probabilities calculated for the different return periods.
...
In summary, the ACF analysis highlights distinct temporal behaviors of covariates. Storm surges exhibit long-term dependencies, wind speed fluctuations are short-lived, temperature is highly persistent due to seasonal effects, and rainfall events occur independently with no significant memory. These differences reflect the underlying physical and meteorological drivers governing each variable.

The CCF plots reveal relationships between water surge and other variables, as shown in @fig-ccf_halifax for Halifax, and in @fig-ccf_yarmouth for Yarmouth found in the Appendix. Since the results for both locations are similar, only the analysis for Halifax is presented below:

- Storm Surge vs. Wind Speed (Top Plot): The CCF shows a positive correlation at lag 0, with a value of approximately 0.2125, followed by a rapid decline in subsequent lags. This suggests wind events immediately impact water surge, with effects persisting through multiple time periods. The pattern indicates wind speed is a driver of water surge events.

- Storm Surge vs. Temperature (Middle Plot): A strong negative correlation is observed, peaking at lag 3 with a CCF value around -0.2058. This suggests that increases in temperature are followed by decreases in storm surge levels after a few days. This inverse relationship is particularly pronounced for Halifax. That suggests lower temperatures are associated with higher storm surge events (possibly related to winter storm patterns). 

- Storm Surge vs. Rainfall (Bottom Plot): The CCF reveals a short-term positive correlation, with the highest value of about 0.1845 occurring at lag 0. The relationship weakens quickly over time, indicating that rainfall has a short-term impact on water surges but little long-term influence.

![CCF plots of storm surge vs wind speed (top), storm surge vs temperature (middle), storm surge vs rain (bottom) in Halifax.](../visualization/ccf_halifax.png){width=60% #fig-ccf_halifax fig-pos="H"}
 
The patterns are generally similar between Halifax and Yarmouth, suggesting regional consistency in meteorological relationships. Overall, the findings suggest that storm surge events in Nova Scotia are primarily driven by wind conditions, with secondary influences from temperature variations and short-term rainfall. 
...
![GAMMs output of LME](../visualization/gamms_lme.JPG){width=100% #fig-gamms_lme fig-pos="H"}

![GAMMs output of GAM](../visualization/gamms_gam.jpeg){width=100% #fig-gamms_gam fig-pos="H"}
...
# View the ordered p-values, first 10 covariates with lowest p-values
head(halifax_ordered_surge, 10)
```

+ This model explained approximately 63.1% of the variation in storm surge with `R-squared = 0.631` and exhibited the AIC of -39,456.38. 

+ Among the most significant predictors, the strongest influence is observed for the storm surge level at lag 1, with an estimated coefficient of 0.6152. This indicates a strong positive relationship, suggesting that current storm surge levels are influenced by the value from the day before.

+ Wind speed has a negative effect at lag 1, with -0.00175, and a positive effect at lag 2, with 0.000974, on storm surge. These smaller coefficients are due to wind speed being measured in km/h, which tends to have larger numerical values.

+ Rainfall displays a negative effect at lag 2, with -1.14e-03, and a positive effect for lag 1, with 9.46e-04. As with wind speed, the smaller coefficients are a result of rainfall being measured in mm, which also involves relatively high magnitudes.

+ Temperature also shows a negative effect at lag 2, with -2.75e-03, and a positive effect for lag 1, 1.86-03. These coefficients are smaller than that of water surge at lag 1, likely due to temperature being measured in degrees Celsius, which again has a larger numerical scale.

+ Additionally, the autocorrelation function (ACF) in @fig-hf_VAR_acf_res indicates no significant autocorrelation except lag 0, confirming that the model effectively captured temporal dependencies.

![ACF plot of residuals of VAR(7) model for Halifax.](../visualization/hf_VAR_acf_res.png){width=50% #fig-hf_VAR_acf_res fig-pos="H"}
...
A simplified model using VAR(1) incorporates only water surge and wind speed with a single lag. This is fitted to evaluate whether wind speed as a significant covariate, can be used to forecast water surge one step ahead. The VAR(1) model having AIC of -37,594.7 is larger than the VAR(7) model AIC value of -39,456.38. The relevant output is shown below:
...
# R square = 0.59, AIC = -37,594.7
summary(halifax_var_model2$varresult$avg_water_surge)
```

The VAR(1) model accounts for 59% of the variance in storm surge and has the AIC value of -37,666.21, indicating a relatively poor fit compared to the more complex VAR(7) model. Despite this, both included predictors—storm surge and wind speed—remain highly significant, with `p-value < 2e-16`. Wind speed displays a positive effect, with the estimated coefficient of 7.290e-04, on storm surge levels. The residual autocorrelation, as shown in @fig-hf_VAR_model2_acf_res, appears acceptable, suggesting that the simplified model still captures key temporal dynamics.

![ACF plot of residuals of VAR(1) model of water surge and wind speed in Halifax.](../visualization/hf_VAR_model2_acf_res.png){width=50% #fig-hf_VAR_model2_acf_res fig-pos="H"}
...
- All three covariates at lag 1 combined: When all three climatic variables are jointly tested at lag 1, the null hypothesis that they do not Granger-cause storm surge is strongly rejected (`F = 118.31, p < 2.2e-16`). This result indicates that the lagged climatic factors provide significant predictive information for storm surge.

- Wind Speed at lag 1 significantly improves model performance, showing the strongest Granger causality effect (`F-Test = 333.05, p < 2.2e-16`). This suggests wind speed at lag 1 is the most dominant predictor of storm surge variation.

- Rainfall at lag 1 also significantly contributes to the predictive model for storm surge dynamics (`F-Test = 89.911, p < 2.2e-16`), suggesting a causal relationship. 

- Temperature although weaker than the other variables, still shows a significant effect at lag 1 (`F-Test = 41.206, p = 1.387e-10`), supporting its predictive relevance.
...
The extraction of annual maxima for each location enables the visualization of extreme values over time. As outlined in the Analysis section, these annual maxima are modeled using the Generalized Extreme Value (GEV) distribution without incorporating the wind speed covariate. @fig-eva_halifax and @fig-evadiag_halifax display the annual maxima over time and the corresponding GEV diagnostic plots, respectively.

![Halifax Annual Maximas Plotted against time](../visualization/evaannual_halifax.png){width=70% #fig-eva_halifax fig-pos="H"}

![Halifax Diagnostics plots from the GEV fit - Non Covariate Case](../visualization/evadiag_halifax.png){width=90% #fig-evadiag_halifax fig-pos="H"}

From these figures, two key observations can be made:

- The annual maximas exhibit a subtle positive linear trend over time. This suggests the presence of non-stationarity inherent in the response variable data, storm surge. Although this non-stationarity likely contributes to some uncertainty in the model results, addressing it falls outside the scope of this project.
...
- $\hat\mu = 0.46$ is the baseline strom surge level expected annually. 
...
Using the obtained MLE estimates, return level plots corresponding to return periods of $2$, $5$, $10$, $20$, $50$, and $100$ years can be constructed. @fig-returnlev_halifax presents the return level plot for storm surges in Halifax, including confidence intervals for each return level estimate.

![Halifax Return Level Plot - Non Covariate Case](../visualization/returnlev_halifax.png){width=70% #fig-returnlev_halifax fig-pos="H"}

Table~\ref{tab:return-periods} below presents the return level estimates in greater detail, complementing the information shown in @fig-returnlev_halifax.
...
Looking at the overall pattern, both locations display higher storm surge return levels for the longer return periods, following a logarithmic curve that flattens at higher return periods. This is typical in extreme value analysis with the difference between a 50-year and 100-year event being less dramatic than between a 2-year and 10-year event. Comparing both location, Halifax consistently shows higher return levels (approximately 0.08-0.10m higher) compared to Yarmouth across all return periods. For example, Halifax's 100-year return level reaches about 0.71m while Yarmouth's reaches about 0.62m. See @fig-eva_yarmouth, @fig-evadiag_yarmouth, @fig-returnlev_yarmouth, and Table~\ref{tab:yarmouth-return-periods} for references to the duplicated process for the Yarmouth location in the appendix.

Now, with respect to the incorporation of the covariates, the storm surge return levels were computed as explained in the above Analysis section. @fig-windspeedplot_halifax displays a visual of the wind speed data, while @fig-windspeedreturnlvl_halifax is the visual plot of the resulting return levels. See @fig-windspeedplot_yarmouth, @fig-windspeedreturnlvl_yarmouth, @fig-returnlev_yarmouth, and Table~\ref{tab:yarmouth-return-periods-covariate} for references to the duplicated process for the Yarmouth location in the appendix.

![Halifax Wind Speed Plot](../visualization/windspeedplot_halifax.png){width=70% #fig-windspeedplot_halifax fig-pos="H"}

![Halifax Return Level Plot - Covariate Case](../visualization/windspeedreturnlvl_halifax.png){width=70% #fig-windspeedreturnlvl_halifax fig-pos="H"}


Table~\ref{tab:return-periods-covariate} below presents the return level estimates in greater detail, complementing the information shown in @fig-windspeedreturnlvl_halifax.
...
It is clear that the covariate analysis produced is remarkably similar results to the initial analysis, with differences so minimal they are indistinguishable at the hundredth decimal place displayed in Table~\ref{tab:return-periods} and Table~\ref{tab:return-periods-covariate}. For Halifax, differences between initial and covariate models are extremely small (0.0001m-0.0003m). For Yarmouth, differences are slightly larger but still minor (0.0049m-0.0116m). Both graphs show upper and lower confidence bounds. The confidence interval widens as the return period increases, reflecting greater uncertainty in predicting later extreme events. 

The minimal difference between the initial and covariate models suggests that wind speed does not significantly enhance the prediction of storm surges beyond what is already captured by the initial EVA model with no covariate. This may indicate that wind speed is implicitly reflected in the storm surge data, that the relationship between wind speed and storm surge is not strong enough to improve predictions, or that other environmental factors play a more critical role in driving extreme storm surge events. More plausibly, the similarity in results may stem from a suspected incorrect implementation of the wind speed covariate in the model, as mentioned in an earlier section. Given this uncertainty, future work could explore forecasting of storm surge levels using wind speed as a covariate through alternative methods—such as Long Short-Term Memory (LSTM) networks—which may offer smoother implementation and improved predictive performance.
...
As part of the continued analysis addressing question two, the EVA suggests that Halifax generally experiences slightly higher storm surge extremes than Yarmouth. However, incorporating wind speed as a covariate resulted in minimal to no improvement in predictive performance. This indicates that either a more accurate implementation of the covariate within the EVA framework, or the use of an alternative modeling approach, may be more effective for forecasting extreme events—an idea that will be explored further in the following section.

## 6.2 Discussion

Initially, this study aimed to analyze storm surge dynamics across multiple locations in Nova Scotia. However, due to limited data availability, only two locations were included. Missing values in both storm surge and covariate data posed a challenge. Accounting for the missingnesss in our storm surge variable would require other methods such as the Bayesian Hierarchial Model, which is beyond the scope of this project. While linear interpolation was applied to impute missing covariate values, it does not account for uncertainty, which may introduce bias. To preserve data integrity, we only used covariate values corresponding to available surge data. Additionally, we assumed equal time spacing between observations, which may affect estimation accuracy.

For the first question, regarding the interaction between covariates and storm surge levels using the VAR model, one limitation is the assumption of a fixed lag structure over time. This may not hold over long periods, potentially affecting the model’s validity. Additionally, the VAR model's assumption of stationarity may not fully capture the evolving nature of the underlying climatic processes. Moreover, although Granger causality tests indicate statistical dependence, they do not imply true causality and are limited to detecting linear relationships. 

For the second question, the aim was to use question one results to improve storm surge return level estimates. Initially only storm surge data was used for the predictions with the intention to make comparison for when there is incorporation of the covariate. Results were not as expected, which raised several discussion points regarding the analysis:
...
-	Additionally, a flawed implementation could be the cause of this unsatisfactory results. Even when applying the formula as suggested in the description of the used R function, it is believed that incorporating covariates requires a more complex approach than initially assumed. Either a more correct implementation can be completed or a different method such as Long Short-Term Memory (LSTM) models may serve as an alternative. Because LSTMs can capture long-term dependencies and may provide more effective predictions for extreme surge events.

- A critical aspect of the model, which was briefly indicated, is the existing non-stationarity as part of the nature of the storm surge data. Our response variable, storm surge, is influenced by sea level, which is known to be increasing over time. Consequently, storm surge is also expected to rise over time. Despite this and for simplicity, non-stationarity was not addressed as it is expected to be a much more complicated procedure. 

**Disclaimer: Chat GPT was utilized in rewording our own words, and we extracted what was suitable for us.** 
...
adf_test_ym <- apply(yarmouth_var, 2, adf.test)
adf_test_ym
```
...
# View the ordered p-values, first 10 covariates with lowest p-values
head(yarmouth_ordered_surge, 10)
```

![ACF plot of residuals of VAR(7) model for Yarmouth.](../visualization/ym_VAR_acf_res.png){width=50% #fig-ym_VAR_acf_res fig-pos="H"}
...
![ACF plot of residuals of VAR(1) model of water surge and wind speed in Yarmouth.](../visualization/ym_VAR_model2_acf_res.png){width=50% #fig-ym_VAR_model2_acf_res fig-pos="H"}
...
![ACF plot of residuals of AR(1) model of storm surge level in Yarmouth.](../visualization/ym_acf_res_surgel1.png){width=50% #fig-ym_acf_res_surgel1 fig-pos="H"} 
...
causality(yarmouth_var_model, c('avg_wind_speed', 'avg_temperature', 'rain'))$Granger

print("Wind speed")
yarmouth_var_model_wind <- VAR(yarmouth_var[, c("avg_water_surge", "avg_wind_speed")], p = 1, type = "none")
causality(yarmouth_var_model_wind, 'avg_wind_speed')$Granger

print("Temperature")
yarmouth_var_model_temp <- VAR(yarmouth_var[, c("avg_water_surge", "avg_temperature")], p = 1, type = "none")
causality(yarmouth_var_model_temp, 'avg_temperature')$Granger

print("Rain")
yarmouth_var_model_rain <- VAR(yarmouth_var[, c("avg_water_surge", "rain")], p = 1, type = "none")
causality(yarmouth_var_model_rain, 'rain')$Granger
```

![Yarmouth Annual Maximas Plotted against time](../visualization/evaannual_yarmouth.png){width=70% #fig-eva_yarmouth fig-pos="H"}

![Yarmouth Diagnostics plots from the GEV fit - Non Covariate Case](../visualization/evadiag_yarmouth.png){width=90% #fig-evadiag_yarmouth fig-pos="H"}

![Yarmouth Return Level Plot - Non Covariate Case](../visualization/returnlev_yarmouth.png){width=70% #fig-returnlev_yarmouth fig-pos="H"}
...
![Yarmouth Wind Speed Plot](../visualization/windspeedplot_yarmouth.png){width=70% #fig-windspeedplot_yarmouth fig-pos="H"}

![Yarmouth Return Level Plot - Covariate Case](../visualization/windspeedreturnlvl_yarmouth.png){width=70% #fig-windspeedreturnlvl_yarmouth fig-pos="H"}
