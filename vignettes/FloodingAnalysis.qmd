...
This study aims to model storm surge levels in Nova Scotia using wind speed, temperature, and rainfall as climatic covariates. It seeks to address the following research questions:
...
+ This model explained approximately 63.1% of the variation in storm surge (`R-squared = 0.631`) and exhibited the AIC of -39,456.38, suggesting the moderate well fit. 
...
+ Additionally, residual analysis using the autocorrelation function (ACF) plot (@fig-hf_VAR_acf_res) indicates no significant autocorrelation, confirming that the model effectively captured temporal dependencies.
...
A simplified model using VAR(1) incorporates only water surge and wind speed with a single lag is fitted to evaluate whether wind speed, as a significant covariate, can be used to forecast water surge one step ahead. The model having AIC of -37,594.7 with the output shown as follows:
...
To further isolate the influence of past water surge levels, the first order autoregressive (AR) model was estimated - AR(1). The result is shown as follows:
